...
- Claude Code: open `/mcp`, select `nuday-complete-readonly`, and authenticate.
- Codex: select **Authenticate** in the MCP server list or run
  `codex mcp login nuday-complete-readonly`.
- Cursor: enable the server and complete the sign-in prompt.
- OpenCode: run `opencode mcp auth nuday-complete-readonly` if it does not prompt on
  first use.
...
- All enabled, public platform skills from NuDay Manager's catalog, rendered
  ...
  Manager.
- One MCP connection to `https://app.nuday.ai/mcp/complete-readonly` for
